    ...
    try:
        ...
        if request.method == "PATCH":
            ...
            if 'value' not in data:
                result["returncode"]=1
                result["errormsg"]="Data must contain key 'value'"
                result["status"]="FAILED"
                result["msg"]="" 
                return JsonResponse(result)
            ...
            if 'scope' not in data:
                result["returncode"]=1
                result["errormsg"]="Data must contain key 'scope'"
                result["status"]="FAILED"
                result["msg"]="" 
                return JsonResponse(result)  
            ...
            if isinstance(value, int):
                logger.debug("alter system set %s=%s scope=%s" %(data['name'], value, data['scope']))
                cursor.execute("alter system set %s=%s scope=%s" %(data['name'], value, data['scope']))
            elif isinstance(value, str):
                logger.debug("alter system set %s='%s' scope=%s" %(data['name'], value, data['scope']))
                cursor.execute("alter system set %s='%s' scope=%s" %(data['name'], value, data['scope']))
            else:
                result["returncode"]=1
                result["errormsg"]="Unknown type"
                return JsonResponse(result)
